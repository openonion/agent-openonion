---
allowed-tools: Bash, Read, Edit, TodoWrite, WebSearch, Grep, Glob, Skill
argument-hint: [patch|minor|major|auto]
description: Automate version release - update version, build, publish to PyPI, create GitHub release, update docs, and post announcement tweet
---
...
### Files to Update
1. `/pyproject.toml` - `version` field (single source of truth)
2. `/connectonion/__init__.py` - `__version__` variable
3. `/VERSIONING.md` - Current version section
4. `../docs-site/app/layout.tsx` - `softwareVersion` in JSON-LD schema (separate repo at /platform/docs-site/)
5. `../docs-site/components/DocsSidebar.tsx` - Version badge in sidebar
...
old: __version__ = "0.5.0"
new: __version__ = "0.5.1"

# Example: Update docs-site/app/layout.tsx
old: "softwareVersion": "0.5.0"
new: "softwareVersion": "0.5.1"

# Example: Update docs-site/components/DocsSidebar.tsx
old: <span className="text-xs text-gray-400">v0.5.0</span>
new: <span className="text-xs text-gray-400">v0.5.1</span>
```

**Note:** docs-site is a separate git repo. After updating, commit and push:
```bash
cd ../docs-site
git add app/layout.tsx components/DocsSidebar.tsx
...
```bash
git add pyproject.toml connectonion/__init__.py VERSIONING.md
...
### Build Package
```bash
# Using hatchling (modern build system defined in pyproject.toml)
python -m build
```
...
Post release announcement to X using the `/tweet` skill:

**IMPORTANT**: After completing all release steps (PyPI upload, GitHub release, Discord notification), use the Skill tool to post a tweet announcing the release.
...
📦 PyPI: https://pypi.org/project/connectonion/{VERSION}/
🔗 Release notes: https://github.com/openonion/connectonion/releases/tag/v{VERSION}
📖 Docs: https://docs.connectonion.com

#Python #AI #AgentFramework
```

**How to execute:**
Call the Skill tool with `skill: "tweet"` and pass the formatted tweet content as the args parameter.

Example:
```
Skill tool:
- skill: "tweet"
- args: "🎉 ConnectOnion v0.5.1 is now available!\n\n[rest of tweet content]"
```

The tweet skill will:
1. Format and prepare the content
2. Show you a preview
3. Post to @ConnectOnionAI after confirmation

**Update TodoWrite:**
```
- [x] Sent Discord release announcement
- [x] Posted X (Twitter) announcement
```
...
- [x] Version files updated (pyproject.toml + __init__.py)
- [x] Release notes generated
