...
# Verify branch matches issue number
BRANCH_ISSUE=$(git branch --show-current | grep -o '[0-9]*$')
echo "Branch is for issue #$BRANCH_ISSUE"
echo "You should be working on issue #$ISSUE_NUM"

if [ "$BRANCH_ISSUE" != "$ISSUE_NUM" ]; then
    echo "ERROR: Branch issue ($BRANCH_ISSUE) doesn't match provided issue ($ISSUE_NUM)"
    exit 1
fi
```
